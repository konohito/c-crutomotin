...
/* ログイン画面。Firebase Auth（メール＋パスワード）。
   ...
   - 利用者: 発行された「ログインID」(u+参加者ID)でログイン → 本人の電子手帳
...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    setBusy(true); setErr('')
    try {
      // 「@」の無い入力はログインID(利用者)とみなし、合成メールに変換してログインする
      await signIn(portalEmailOf(email), pw)
    ...
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: 'var(--bg-canvas)' }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div className="card" style={{ padding: '26px 26px 28px' }}>
          <div style={{ fontSize: 18, fontWeight: 700, textAlign: 'center' }}>ログイン</div>
          <div style={{ fontSize: 12.5, color: 'var(--fg-3)', textAlign: 'center', marginTop: 5, lineHeight: 1.6 }}>
            介護予防・体力測定管理システム<br />
            職員はメールアドレス、利用者は発行された<br />ログインIDでログインしてください
          </div>
          <form onSubmit={submit} style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <span style={{ fontSize: 12, color: 'var(--fg-2)', fontWeight: 600 }}>メールアドレス または ログインID</span>
              <input className="field" type="text" autoComplete="username" value={email}
                onChange={(e) => setEmail(e.target.value)} placeholder="staff@example.jp / u10001" autoFocus />
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
