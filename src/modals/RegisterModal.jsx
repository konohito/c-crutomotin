...
export default function RegisterModal() {
  ...
  const save = async () => {
    ...
    const code = wardIdCode(ward) ?? (matched ? matched[0] : (mu.venues && mu.venues[0] ? mu.venues[0][0] : 900))
  ...
  return (
    <Modal onClose={close} width="min(520px, 94vw)">
      ...
      <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <Field label="介護度" hint="(optional)">
            <Select value={state.regCare} onChange={(e) => set({ regCare: e.target.value })} options={CARE_OPTS} style={{ height: 40, fontSize: 13.5 }} />
          </Field>
